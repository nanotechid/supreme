...
ntls="$(cat ~/log-install.txt | grep -w "Port non TLS" | cut -d: -f2|sed 's/ //g')"
...
trojanlink="trojan://${uuid}@${domain}:${tls}?path=/worryfree/trojan&security=tls&host=${domain}&type=ws&sni=${domain}#${user}"
trojanlink2="trojan://${uuid}@${domain}:${ntls}?path=/worryfree/trojan&security=none&host=${domain}&type=ws#${user}"
...
echo -e "Path           : /worryfree/trojan" | tee -a /etc/log-create-user.log
